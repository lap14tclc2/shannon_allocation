...
def test_live_total_pnl_does_not_require_a_snapshot(tmp_path):
    ...
    svc.store.upsert_market_prices([{
        ...
        "volume": 1_000_000, "source": "fake", "is_final": True,
        "data_quality": "VALID",
    }])
    ...
    assert dashboard["portfolio"]["total_return"] == pytest.approx(0.25)
...
def test_extended_risk_diagnostics_are_available_after_sync(tmp_path):
    svc = make_service(tmp_path)
    for symbol, qty in (("AAA", 100), ("BBB", 80), ("CCC", 60)):
        svc.append_event({
            "event_type": "POSITION_IMPORT", "event_date": "2026-01-02",
            "symbol": symbol, "quantity": qty, "price": 20_000,
        })
    svc.sync_daily()
    risk = svc.risk()
    for key in (
        "effective_positions", "average_correlation", "max_correlation",
        "diversification_ratio", "daily_var_95", "daily_cvar_95",
        "downside_volatility", "largest_risk_symbol", "largest_risk_contribution",
    ):
        assert key in risk
