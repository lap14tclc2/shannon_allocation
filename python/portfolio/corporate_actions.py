...
def _stable_event_key(*, source: str, symbol: str, action_type: str, event_name: str | None, record_date: str | None,
    ...
    if native_id not in (None, ""):
        return f"{source}:{symbol}:id:{str(native_id).strip()}:{action_type}"
    material = {
...
def normalize_event_row(row: dict, default_symbol: str | None = None, source: str = "vnstock") -> CorporateAction | None:
    ...
    text = _text(raw); action_type = _classify(text)
    name_value = _value(raw, ("event_name", "event_title", "title", "name", "description", "purpose", "reason"))
    event_name = str(name_value).strip() if name_value not in (None, "") else None
    ...
    cash = _value(raw, ("cash_per_share", "dividend_value", "cash_dividend")); ratio = _value(raw, ("stock_ratio", "ratio", "dividend_ratio"))
    ...
    external_key = _stable_event_key(source=source,symbol=symbol,action_type=action_type,event_name=event_name,record_date=record_date,ex_date=ex_date,payment_date=payment_date,announcement_date=announcement_date,cash_per_share=cash_per_share,stock_ratio=stock_ratio,raw=raw)
    return CorporateAction(external_key=external_key,symbol=symbol,action_type=action_type,event_name=event_name,announcement_date=announcement_date,ex_date=ex_date,record_date=record_date,payment_date=payment_date,cash_per_share=cash_per_share,stock_ratio=stock_ratio,source=source,source_url=str(source_url) if source_url else None,confidence="PROVISIONAL",verification_status="UNVERIFIED",raw=raw)
...
def _from_dividend(event: DividendEvent) -> CorporateAction:
    ...
    if event.source_event_id:
        # One vendor event can contain both cash and stock entitlements (e.g. ACB).
        # Action type is therefore part of the unique key.
        external_key = f"{event.source}:{event.symbol}:id:{event.source_event_id}:{event.dividend_type}"
    else:
        external_key = _stable_event_key(source=event.source,symbol=event.symbol,action_type=event.dividend_type,event_name=event.title,record_date=event.record_date,ex_date=event.ex_date,payment_date=event.payment_date,announcement_date=event.announcement_date,cash_per_share=event.cash_per_share,stock_ratio=event.stock_ratio,raw=raw)
    return CorporateAction(external_key=external_key,symbol=event.symbol,action_type=event.dividend_type,event_name=event.title,announcement_date=event.announcement_date,ex_date=event.ex_date,record_date=event.record_date,payment_date=event.payment_date,cash_per_share=event.cash_per_share,stock_ratio=event.stock_ratio,source=event.source,source_url=event.source_url,confidence="PROVISIONAL",verification_status="UNVERIFIED",raw=raw)


class VnstockCorporateActionProvider:
    """Compatibility name for QPort's resilient corporate-action provider.

    Production discovery order follows the dividend provider chain (public/no-auth
    sources first) and keeps Vnstock as an optional final fallback. Injected
    Reference factories preserve legacy deterministic test behavior.
    """
    ...
    def __init__(self, reference_factory=None, provider_name: str | None = None, dividend_providers=None) -> None:
        ...
        self._dividend_providers = dividend_providers if dividend_providers is not None else ([] if self._injected else default_dividend_providers())
        self._last_attempts: list[dict] = []
        self._last_success_source: str | None = None

    def health(self) -> dict:
        if self._injected:
            return {"provider":self.name,"available":self._Reference is not None,"api_variant":self._api_variant,"isolation":"INJECTED_DIRECT","capability":"REFERENCE_EVENTS"}
    ...
    def _company_events(ref, symbol: str) -> list[dict]:
        ...
        method = getattr(company, "events", None)
        if callable(method):
            for call in (lambda: method(symbol=symbol), lambda: method(symbol)):
                try:
                    rows = _records(call())
                    if rows: return rows
        ...
        if callable(company):
            try:
                obj = company(symbol); legacy = getattr(obj, "events", None)
                if callable(legacy): return _records(legacy())
            except Exception: pass
        return []

    @staticmethod
    def _calendar_events(ref, start: str, end: str) -> list[dict]:
        events_obj = getattr(ref, "events", None); calendar = getattr(events_obj, "calendar", None) if events_obj is not None else None
    ...
    def _injected_events(self, symbols: list[str], start: str, end: str) -> list[CorporateAction]:
        wanted = {str(s).upper() for s in symbols}; ref = self._Reference(); rows = self._calendar_events(ref,start,end)
        if not rows:
            rows = []
    ...
    def events(self, symbols: list[str], start: str, end: str) -> list[CorporateAction]:
        if self._injected: return self._injected_events(symbols,start,end)
        self._last_attempts=[]; self._last_success_source=None; out=[]; seen=set()
        for symbol in sorted({str(s).upper().strip() for s in symbols if s}):
            ...
            for provider in self._dividend_providers:
                try:
                    ...
                    self._last_attempts.append({"symbol":symbol,"provider":provider.name,"status":"SUCCESS" if rows else "EMPTY","count":len(rows)})
                except Exception as exc:
                    self._last_attempts.append({"symbol":symbol,"provider":provider.name,"status":"ERROR","count":0,"error":f"{type(exc).__name__}: {exc}"})
                    continue
            ...
            try:
                ...
                rows = result.get("data") or []; provider_name = str(result.get("provider") or "vnstock"); added=0
                for row in rows:
                    ...
                    if action and action.external_key not in seen:
                        out.append(action); seen.add(action.external_key); added += 1
                self._last_attempts.append({"symbol":symbol,"provider":"vnstock_optional","status":"SUCCESS" if added else "EMPTY","count":added})
                if added: self._last_success_source=provider_name
            except Exception as exc:
                self._last_attempts.append({"symbol":symbol,"provider":"vnstock_optional","status":"ERROR","count":0,"error":f"{type(exc).__name__}: {exc}"})
        return out
