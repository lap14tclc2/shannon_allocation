...
def _generic_dividend_event(
    raw: dict[str, Any], *, symbol: str, source: str, source_url: str, start: str, end: str,
) -> DividendEvent | None:
    text = _row_text(raw)
    ...
    if stock_words:
        ...
        return None
    record_date = _iso_date(_value_ci(raw, (
        "recordDate", "lastRegistrationDate", "last_registration_date", "registrationDeadline", "ngay_dang_ky_cuoi_cung",
    )))
    ex_date = _iso_date(_value_ci(raw, (
        "exrightDate", "exRightDate", "exDate", "registrationDate", "ex_right_date", "ngay_giao_dich_khong_huong_quyen",
    )))
    ...
    return DividendEvent(
        symbol=symbol, dividend_type=dtype, source=source,
        source_event_id=str(native_id) if native_id not in (None, "") else None,
        title=title, announcement_date=announcement_date, ex_date=ex_date, record_date=record_date,
        payment_date=payment_date, cash_per_share=cash_per_share, stock_ratio=stock_ratio,
        source_url=source_url, raw=dict(raw),
    )
...
def _dedupe(events: list[DividendEvent]) -> list[DividendEvent]:
    ...
    for event in events:
        key = (
            event.source_event_id, event.dividend_type, event.record_date, event.ex_date, event.payment_date,
            round(event.cash_per_share or 0.0, 4), round(event.stock_ratio or 0.0, 8), event.title,
        )
        if key not in seen:
            seen.add(key)
            out.append(event)
...
class VpsDividendProvider:
    ...
    def events(self, symbol: str, start: str, end: str) -> list[DividendEvent]:
        ...
        for raw in _walk_dicts(payload):
            event = _generic_dividend_event(raw, symbol=symbol, source=self.name, source_url=url, start=start, end=end)
...
class FireAntPublicContentProvider:
    ...
    def events(self, symbol: str, start: str, end: str) -> list[DividendEvent]:
        ...
        if not _contains_dividend(text):
            return []
        snippets = re.findall(r"[^.!?]{0,180}(?:cổ tức|co tuc|dividend)[^.!?]{0,220}", text, flags=re.IGNORECASE)
        for index, snippet in enumerate(snippets):
            ...
            out.append(DividendEvent(
                ...
                source_event_id=f"html-{index}-{event_date}", title=snippet.strip(), announcement_date=event_date,
                ...
                source_url=url, raw={"snippet": snippet.strip()},
            ))
        return _dedupe(out)


class CafeFDividendProvider:
    """Public CafeF company-event page fallback; parses only explicit dividend/GDKHQ entries."""
    ...
    def _gdkhq_date(text: str) -> str | None:
        match = re.search(r"(\d{1,2}[./-]\d{1,2}[./-]20\d{2})\s*,?\s*ngày\s+GDKHQ", text, re.IGNORECASE)
    ...
    def events(self, symbol: str, start: str, end: str) -> list[DividendEvent]:
        ...
        if not _contains_dividend(text):
            return []
        snippets = re.findall(r"[^.!?]{0,220}(?:cổ tức|co tuc|dividend)[^.!?]{0,260}", text, flags=re.IGNORECASE)
        out: list[DividendEvent] = []
        for snippet in snippets:
            low = snippet.lower()
            ex_date = self._gdkhq_date(snippet)
            date_candidates = re.findall(r"\d{1,2}[./-]\d{1,2}[./-]20\d{2}", snippet)
            announcement_date = _iso_date(date_candidates[0]) if date_candidates else None
            effective = ex_date or announcement_date
            if not effective or not (start <= effective <= end):
                continue
            if any(token in low for token in ("cổ phiếu", "co phieu", "thưởng cp", "stock")):
                dtype = "STOCK_DIVIDEND"
            elif any(token in low for token in ("bằng tiền", "tiền", "tien", "đ/cp", "đồng/cp", "cash")):
                dtype = "CASH_DIVIDEND"
            else:
                continue
            digest = hashlib.sha256(f"{symbol}|{dtype}|{ex_date}|{snippet}".encode("utf-8")).hexdigest()[:20]
            out.append(DividendEvent(
                symbol=symbol, dividend_type=dtype, source=self.name, source_event_id=digest,
                title=snippet.strip(), announcement_date=announcement_date, ex_date=ex_date,
                cash_per_share=_cash_from_title(snippet) if dtype == "CASH_DIVIDEND" else None,
                stock_ratio=_stock_ratio_from_title(snippet) if dtype == "STOCK_DIVIDEND" else None,
                source_url=url, raw={"snippet": snippet.strip()},
            ))
...
class VietcapDividendProvider:
    ...
    def health(self) -> dict:
        return {"provider": self.name, "available": True, "transport": "DIRECT_HTTPS_JSON", "auth_required": "UNKNOWN_OR_PROVIDER_BLOCKED", "priority": "FALLBACK_ONLY"}
    ...
    def events(self, symbol: str, start: str, end: str) -> list[DividendEvent]:
        ...
        for raw in content:
            ...
            if code == "DIV":
                ...
                continue
            record_date = _iso_date(raw.get("recordDate")); ex_date = _iso_date(raw.get("exrightDate"))
            payment_date = _iso_date(raw.get("payoutDate")); announcement_date = _iso_date(raw.get("publicDate"))
            event_date = record_date or ex_date or announcement_date or payment_date
            ...
            out.append(DividendEvent(
                ...
                title=title, announcement_date=announcement_date, ex_date=ex_date, record_date=record_date,
                payment_date=payment_date, cash_per_share=cash, stock_ratio=stock_ratio,
...
class FireAntDividendProvider:
    """FireAnt OAuth Events API kept as a late fallback/cross-check."""
    ...
    def health(self) -> dict:
        return {"provider": self.name, "available": True, "transport": "DIRECT_HTTPS_JSON", "auth_required": True, "priority": "FALLBACK_ONLY"}

    def events(self, symbol: str, start: str, end: str) -> list[DividendEvent]:
        response = self.session.get(self.endpoint, params={
            "symbol": symbol, "type": 0, "orderBy": 2,
            "startDate": f"{start}T00:00:00Z", "endDate": f"{end}T23:59:59Z", "offset": 0, "limit": 200,
        }, headers={"Accept": "application/json", "User-Agent": "Mozilla/5.0 QPort/1.0"}, timeout=self.timeout)
        response.raise_for_status()
        payload = response.json()
        ...
        for raw in rows if isinstance(rows, list) else []:
            ...
            out.append(DividendEvent(
                ...
                title=title, ex_date=_iso_date(raw.get("registrationDate")), record_date=_iso_date(raw.get("recordDate")),
...
def _signature(event: DividendEvent) -> tuple:
    return (
        event.symbol, event.dividend_type, event.record_date, event.ex_date, event.payment_date,
        round(event.cash_per_share or 0.0, 4), round(event.stock_ratio or 0.0, 8),
    )
    ...
    def latest(self, symbol: str, *, years_back: int = 5, years_forward: int = 2) -> dict:
        ...
        for provider in self.providers:
            try:
                ...
                events.extend(rows)
                provider_attempts.append({"provider": provider.name, "status": "SUCCESS" if rows else "EMPTY", "count": len(rows)})
        ...
        base = {
            "ok": True, "symbol": symbol, "source_counts": source_counts, "errors": errors,
            "provider_attempts": provider_attempts,
            "provider_strategy": "FIRST_PROVIDER_WITH_USABLE_DIVIDEND" if self.stop_on_first_data else "AGGREGATE_EVIDENCE",
            "lookback_start": start, "lookahead_end": end, "read_only": True,
        }
        if not events:
        ...
        return {**base, "found": True, "latest": latest_payload, "retrieved_at": datetime.now(timezone.utc).isoformat(timespec="seconds")}
