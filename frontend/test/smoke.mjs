...
const performance={returns:{daily:.01,mtd:.03,ytd:.1,since_inception:.12},annualized_twr:.14,xirr:.11,xirr_status:'AVAILABLE',cashflow_history_quality:'COMPLETE',history_status:'SUFFICIENT',total_pnl:1000000,accounting_return:1/23,unrealized_pnl:1000000,realized_pnl:0,dividend_income:200000,net_dividend_income:190000,cash_dividend_tax:10000,stock_dividend_sale_tax:0,fees_and_taxes:10000,net_external_contributions:23000000,cash:10000000,equity_value:14000000,nav:24000000,current_drawdown:-.02,max_drawdown:-.08,snapshot_count:120,official_snapshot_count:118,first_date:'2026-01-02',latest_date:'2026-08-21',best_day:.035,worst_day:-.028,positive_day_ratio:.54,latest:{nav:24000000},series:[{date:'2026-08-20',nav:23700000},{date:'2026-08-21',nav:24000000}],methodology_policy:{policy_version:'QPORT_PERF_V2',cost_method:'FIFO_TAX_LOTS'}};
const risk={status:'VALID',volatility_63:.21,volatility_252:.24,volatility_ratio:.875,max_position_weight:.5833,max_equity_weight:1,equity_hhi:1,effective_positions:1,effective_position_ratio:1,n_positions:1,average_correlation:null,max_correlation:null,diversification_ratio:1,daily_var_95:-.021,daily_cvar_95:-.031,max_daily_loss:-.049,downside_volatility:.19,positive_day_ratio:.54,return_observations:240,largest_risk_symbol:'FPT',largest_risk_contribution:1,equal_risk_contribution:1,risk_concentration_ratio:1,risk_contribution_hhi:1,risk_contributions:{FPT:1},erc_reference_weights:{FPT:1},quality:{coverage_weight:1,requested_symbols:1,eligible_symbols:1,missing_symbols:[]},methodology:{concentration_basis:'equity_normalized'}};
...
if(existsSync(ssrEntry)){
  ...
  check('performance renders richer decision-useful overview',perfHtml.includes('Total P/L')&&perfHtml.includes('Since inception TWR')&&perfHtml.includes('Annualized TWR')&&perfHtml.includes('Best / worst day')&&perfHtml.includes('P/L bridge'));
  check('performance exposes dividend tax impact',perfHtml.includes('Income after dividend tax')&&perfHtml.includes('5% cash-dividend withholding')&&perfHtml.includes('Net cash-dividend income'));
  check('performance exposes history readiness',perfHtml.includes('History readiness')&&perfHtml.includes('Official snapshots')&&perfHtml.includes('XIRR'));
  check('performance keeps methodology behind disclosure',perfHtml.includes('How QPort measures performance')&&perfHtml.includes('Current / max drawdown'));
  const riskHtml=renderPage('risk',{risk,locale:'en'});
  check('risk advanced route renders interpretation concentration and tail risk',riskHtml.includes('Risk interpretation')&&riskHtml.includes('Correlation regime')&&riskHtml.includes('Volatility regime')&&riskHtml.includes('Equity HHI')&&riskHtml.includes('Daily CVaR 95%'));
  ...
  check('transactions show broker account and tax columns',txHtml.includes('Advanced details')&&txHtml.includes('Broker')&&txHtml.includes('Account')&&txHtml.includes('Fee / Tax')&&txHtml.includes('TCBS'));
  check('transactions remove institutional clutter',!txHtml.includes('Institutional ledger rules')&&!txHtml.includes('Correction audit log')&&!txHtml.includes('Inline edits create audited corrections'));
  ...
  check('operations remains available as advanced route',opsHtml.includes('Operations &amp; Book Controls')&&opsHtml.includes('Settlement book')&&opsHtml.includes('Tax lots · FIFO')&&opsHtml.includes('Broker reconciliation')&&opsHtml.includes('Corporate actions')&&opsHtml.includes('NAV controls &amp; restatement'));
  ...
  check('logs remain available as advanced route',logsHtml.includes('Activity Log')&&logsHtml.includes('VERIFIED')&&logsHtml.includes('TRANSACTION_CORRECTED')&&logsHtml.includes('hash chain'));
