...
export default function PortfolioAssessmentEnhancer({ dashboard = {}, locale = 'en' }) {
  ...
  useEffect(() => {
    if (typeof document !== 'undefined') {
      setTarget(document.querySelector('.portfolio-assessment-card'));
    }
  }, []);

  const evidence = useMemo(() => {
    const coverage = Number(quality.coverage_weight || 0);
    const requested = Number(quality.requested_symbols ?? positions.length ?? 0);
    const eligible = Number(quality.eligible_symbols ?? Math.max(0, requested - (quality.missing_symbols || []).length));
    ...
    return { coverage, requested, eligible, historyCount, returnObs, riskReady, performanceReady };
  }, [quality, perf, risk, positions.length]);

  if (!target || positions.length === 0) return null;

  return createPortal(
    <div className="assessment-depth" data-testid="portfolio-assessment-depth">
      ...
      <div className="assessment-depth-grid">
        ...
        <section className="assessment-depth-block">
          ...
          <div className="diag-row"><span>{text('Largest risk contributor', 'Đóng góp risk lớn nhất')}</span><b>{risk.largest_risk_symbol || '-'} {pct(risk.largest_risk_contribution)} · {num(risk.risk_concentration_ratio)}× {text('equal-risk', 'mức cân bằng')}</b></div>
        </section>
