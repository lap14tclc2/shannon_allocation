...
export async function downloadBusinessMungerAIExport(data) {
  ...
  const lines = [
    ...
    `- **Trạng thái**: **${formatDecision(decision.state) || decision.state_vietnamese || 'Chờ biên an toàn'}** (${decision.state || 'WAIT_FOR_MOS'})`,
    ...
    ``,
    `## 4. Thanh Khoản Giao Dịch Thực Tế (Liquidity Gate)`,
    `- **Phân loại thanh khoản**: **${munger.liquidity?.classification_vi || 'Chưa đủ dữ liệu thanh khoản'}** (${munger.liquidity?.classification || 'LIQUIDITY_INSUFFICIENT_DATA'})`,
    `- **Giá giao dịch gần nhất**: ${money(munger.liquidity?.latest_price || val.current_price)}`,
    `- **KLGD bình quân 20 phiên**: ${munger.liquidity?.avg_volume_20d != null ? `${num(munger.liquidity.avg_volume_20d, 0)} cp/ngày` : 'Chưa đủ dữ liệu'}`,
    `- **GTGD bình quân 20 phiên**: ${(munger.liquidity?.avg_trading_value_20d_billion ?? munger.liquidity?.avg_trading_value_20d) != null ? `${munger.liquidity.avg_trading_value_20d_billion ?? munger.liquidity.avg_trading_value_20d} tỷ đ/ngày` : 'Chưa đủ dữ liệu'}`,
    `- **Mật độ phiên có giao dịch**: ${munger.liquidity?.trading_day_coverage_pct != null ? `${munger.liquidity.trading_day_coverage_pct}% (${munger.liquidity.trading_days_found ?? munger.liquidity.trading_days_observed ?? 0}/${munger.liquidity.trading_days_observed ?? 20})` : 'Chưa đủ dữ liệu'}`,
    `- **Nhận xét thanh khoản**: ${munger.liquidity?.commentary_vi || 'Chưa đủ dữ liệu thanh khoản để đánh giá.'}`,
    ``,
    `## 5. Ma Trận Chất Lượng Tài Chính 12 Chiều Munger`,
    table(
    ...
    ``,
    `## 7. Đánh Giá Bẫy Giá Trị (Value Trap Assessment)`,
    ...
    ``,
    `## 8. Forensic Findings & Pre-Commitment Checklist`,
    ...
    ``,
    `## 9. Machine-Readable Payload for AI`,
    `\`\`\`json`,
