...
export default function GuidePage({locale='en'}){
  ...
  return <div className="page">
    ...
    <div className="card"><h3>{t('guide.quick_start')}</h3><pre className="guide-code"><code>{`cd frontend\nnpm ci\nnpm run build\nnpm run build:ssr\n\ncd ../python\npip install -r requirements.txt\n# Optional Vnstock + corporate actions:\n# pip install -r requirements-vnstock.txt\n# First use: configure admin securely in the terminal\npython -m portfolio.cli setup-admin\npython serve.py`}</code></pre></div>

    <div className="card"><h3>{text('Authentication & first sign-in','Xác thực & đăng nhập lần đầu')}</h3><div className="rule-list">
      <div>✓ {text('Normal users sign in with a unique username only. No password is required for a normal user. Keep this mode on localhost or a trusted private network.','User thường đăng nhập chỉ bằng username duy nhất. User thường không cần password. Chỉ dùng chế độ này trên localhost hoặc mạng riêng tin cậy.')}</div>
      ...
      <div>✓ {text('Admin has no password embedded in source or UI. Configure the admin password locally with python -m portfolio.cli setup-admin before the first admin login.','Admin không có password hard-code trong source hoặc UI. Hãy cấu hình password admin cục bộ bằng python -m portfolio.cli setup-admin trước lần đăng nhập admin đầu tiên.')}</div>
