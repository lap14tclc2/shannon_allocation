...
export default function OperationsPage({ operations = {}, today = '', locale = 'en' }) {
  ...
  async function lookupDividend() {
    ...
    try {
      const result = await getLatestDividend(symbol);
  ...
  const latestDividend = dividendResult?.latest || null;
  ...
  return <div className="page">
    ...
    <section className="card">
      <div className="section-head"><div><h3>{text('Latest dividend lookup','Tra cổ tức mới nhất')}</h3><p className="muted">{text('Read-only lookup using direct Vietcap IQ first and FireAnt as independent fallback/cross-check. It never changes cash or shares.','Tra cứu read-only: ưu tiên Vietcap IQ trực tiếp và dùng FireAnt làm fallback/cross-check độc lập. Chức năng này không thay đổi tiền hay cổ phiếu.')}</p></div></div>
      ...
      {latestDividend&&<div className="table-scroll"><table className="ranking"><thead><tr><th>{text('Ticker','Mã')}</th><th>{text('Type','Loại')}</th><th>{text('Ex-date','Ngày GDKHQ')}</th><th>{text('Record date','Ngày ĐKCC')}</th><th>{text('Payment date','Ngày thanh toán')}</th><th>{text('Value / ratio','Giá trị / tỷ lệ')}</th><th>{text('Source','Nguồn')}</th><th>{text('Cross-check','Đối chiếu')}</th></tr></thead><tbody><tr><td><b>{latestDividend.symbol}</b></td><td>{latestDividend.dividend_type}</td><td>{latestDividend.ex_date||'-'}</td><td>{latestDividend.record_date||'-'}</td><td>{latestDividend.payment_date||'-'}</td><td>{latestDividend.dividend_type==='CASH_DIVIDEND'?(latestDividend.cash_per_share!=null?`${money(latestDividend.cash_per_share)}/CP`:'-'):(latestDividend.stock_ratio_percent!=null?`${latestDividend.stock_ratio_percent.toFixed(2)}%`:'-')}</td><td>{latestDividend.source}<div className="muted">{latestDividend.title||''}</div></td><td>{latestDividend.cross_source_match?text('MATCHED','KHỚP NHIỀU NGUỒN'):text('Single source','Một nguồn')}<div className="muted">{(latestDividend.evidence||[]).map(x=>x.source).join(', ')}</div></td></tr></tbody></table></div>}
    </section>

    <section className="card"><div className="section-head"><div><h3>{text('Corporate actions','Quyền doanh nghiệp')}</h3><p className="muted">{text('Stored corporate actions require explicit verification, receipt and ledger posting; no detected event changes the book automatically.','Corporate action đã lưu vẫn cần xác minh, ghi thực nhận và post ledger explicit; event phát hiện không tự thay đổi sổ.')}</p></div><button className="btn-export" disabled={busy} onClick={()=>run(()=>syncCorporateActions())}>{text('Sync events','Đồng bộ event')}</button></div><div className="muted">Provider: {operations.corporate_action_provider?.provider || '-'} · {operations.corporate_action_provider?.available?'AVAILABLE':'UNAVAILABLE'}</div><div className="table-scroll"><table className="ranking corporate-action-table"><thead><tr><th>ID</th><th>{text('Ticker','Mã')}</th><th>{text('Type','Loại')}</th><th>{text('Record date','Ngày ĐKCC')}</th><th>{text('Expected','Dự kiến')}</th><th>{text('Verify source URL','URL xác minh')}</th><th>{text('Actual receipt','Thực nhận')}</th><th>Status</th><th>{text('Actions','Thao tác')}</th></tr></thead><tbody>{actions.map(a=>{const edit=caValue(a);return <tr key={a.id}><td>{a.id}</td><td>{a.symbol}</td><td>{a.action_type}</td><td>{a.record_date||'-'}</td><td>{a.expected_cash!=null?money(a.expected_cash):a.expected_shares!=null?`${formatShares(a.expected_shares,locale)} shares`:'-'}</td><td><input className="inline-wide" value={edit.source_url} onChange={e=>setCA(a,'source_url',e.target.value)} placeholder="https://vsd.vn/..."/><div className="muted">{a.verification_status}</div></td><td><div className="inline-stack"><input value={edit.actual_cash} onChange={e=>setCA(a,'actual_cash',e.target.value)} placeholder={text('Cash','Tiền')}/><input type="number" min="0" value={edit.actual_shares} onChange={e=>setCA(a,'actual_shares',e.target.value)} placeholder={text('Shares','CP')}/></div></td><td>{a.status}<div className="muted">{a.ledger_posted?`POSTED #${(a.postings||[]).map(p=>p.event_id).join(', #')}`:'NOT POSTED'}</div></td><td><div className="inline-stack"><button className="btn-small" disabled={busy||!edit.source_url.trim()} onClick={()=>run(()=>verifyCorporateAction(a.id,edit.source_url.trim()))}>{text('Verify','Xác minh')}</button><button className="btn-small" disabled={busy} onClick={()=>recordReceipt(a)}>{text('Record receipt','Ghi thực nhận')}</button>{a.status==='RECONCILED'&&a.verification_status==='VERIFIED'&&!a.ledger_posted&&<button className="btn-export btn-small" disabled={busy} onClick={()=>run(()=>postCorporateActionReceipt(a.id))}>{text('Post to ledger','Post vào ledger')}</button>}</div></td></tr>})}</tbody></table></div></section>
